...
class customfields {
    function formCustomFields($field) {
        ...
        $cssclass = "";
        $html = '';
        $div1 = (is_admin()) ? 'js-form-wrapper' : 'js-col-md-12 js-form-wrapper';
        ...
        switch ($field->userfieldtype) {
            case 'text':
            case 'email':
                $html .= JSSTformfield::text($field->field, $value, array('class' => 'inputbox one', 'data-validation' => $cssclass, 'size' => $size, 'maxlength' => $maxlength));
                break;
            case 'date':
    ...
    function formCustomFieldsForSearch($field, &$i, $isadmin = 0) {
        ...
        $i++;
        $required = $field->required;
        $div1 = 'js-col-md-12 js-filter-wrapper';
        $div3 = 'js-col-md-6 js-filter-value';
        ...
        switch ($field->userfieldtype) {
            case 'text':
            case 'email':
                $html .= JSSTformfield::text($field->field, $value, array('class' => 'inputbox one', 'data-validation' => $cssclass,'placeholder' =>$field->fieldtitle, 'size' => '.$field->size.', $maxlength, $readonly));
                break;
            case 'date':
                $html .= JSSTformfield::text($field->field, $value, array('class' => 'custom_date one', 'data-validation' => $cssclass));
                break;
    ...
    function userFieldsForSearch($fieldfor) {
        ...
        $query = "SELECT required,field,fieldtitle,isuserfield,userfieldtype,userfieldparams,depandant_field  FROM " . jssupportticket::$_db->prefix . "js_ticket_fieldsordering WHERE isuserfield = 1 AND " . $inquery . " AND fieldfor =" . $fieldfor ." ORDER BY ordering ";
        $data = jssupportticket::$_db->get_results($query);
        return $data;
    }
