				...
				foreach($rawnames as $k => $rawname)
				{
					...
					elseif($type == 'table')
					{
						...
						if($manager)
						{
							...
							{		
								$thefiles .=	
									"<a href='' id='rename-ssfa-file-$uid-$count' title='Rename'>".__('', 'file-away')."<i class='fa fa-pencil'></i></a><br>".// Rename
									"<a href='' id='delete-ssfa-file-$uid-$count' title='Delete'>".__('', 'file-away')."<i class='fa fa-times'></i></a>"; //Delete
							}
							$thefiles .= "</td>"; 
						}
						$thefiles .= '</tr>'; 
