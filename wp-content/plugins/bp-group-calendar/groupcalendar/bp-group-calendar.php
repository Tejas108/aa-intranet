...
class BP_Group_Calendar_Widget_User_Groups extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		$events = $wpdb->get_results( "SELECT gc.id, gc.user_id, gc.event_title, gc.event_time, gp.name, gp.slug FROM ".$wpdb->base_prefix."bp_groups_calendars gc JOIN ".$wpdb->base_prefix."bp_groups gp ON gc.group_id=gp.id WHERE gc.event_time >= '".date('Y-m-d H:i:s')."' AND gp.id IN ($group_ids) ORDER BY gc.event_time ASC LIMIT ".(int)$instance['num_events'] );
		...
		<div class="widget-error">
			<?php _e('There are no upcoming events for your departments.', 'groupcalendar') ?>
		</div>
	<?php } ?>
